...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, callback) {
    ...
    const newFilename =
      file.originalname.replace(new RegExp(fileExtName), "") +
      "-" +
      Date.now() +
      fileExtName;
...
app.get("/", (req, res) => {
  fs.readdir(uploadsPath, function(err, items) {
    ...
    const itemPaths = items
      ...
      .sort(
        (mTimeAndPath1, mTimeAndPath2) => mTimeAndPath2[0] - mTimeAndPath1[0]
      )
...
app.post("/latest", (req, res) => {
  ...
  fs.readdir(uploadsPath, function(err, items) {
    ...
    if (newImageMtimesAndPaths.length > 0) {
      ...
      const newImagePaths = newImageMtimesAndPaths.map(
        ([imageMtime, imagePath]) => imagePath
      );
      res.send({
